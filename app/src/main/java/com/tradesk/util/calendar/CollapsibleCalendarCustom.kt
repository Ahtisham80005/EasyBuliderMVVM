package com.tradesk.util.calendar
...
class CollapsibleCalendarCustom : UICalendarCustom, View.OnClickListener {
    override fun changeToToday() {
        ...
        this.selectedDay = Day(today.get(Calendar.YEAR), today.get(Calendar.MONTH), today.get(
    ...
    fun prevMonth() {
        ...
        params.let {
            if (it != null && (Calendar.getInstance().get(Calendar.YEAR) * 12 + Calendar.getInstance().get(
                    Calendar.MONTH) + it.prevDays / 30) > (cal.get(Calendar.YEAR) * 12 + cal.get(
                    Calendar.MONTH))) {
    ...
    fun nextMonth() {
        ...
        params.let {
            if (it != null && (Calendar.getInstance().get(Calendar.YEAR) * 12 + Calendar.getInstance().get(
                    Calendar.MONTH) + it.nextDaysBlocked / 30) < (cal.get(Calendar.YEAR) * 12 + cal.get(
                    Calendar.MONTH))) {
                val myAnim = AnimationUtils.loadAnimation(context, R.anim.bounce)
                val interpolator = BounceAnimator(0.1, 10.0)
                myAnim.setInterpolator(interpolator)
    ...
    fun setStateWithUpdateUI(state: Int) {
        this@CollapsibleCalendarCustom.state = state
