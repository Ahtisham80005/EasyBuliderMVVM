...
import com.tradesk.activity.clientModule.AddClientActivity
import com.tradesk.databinding.ActivityCustomerDetailBinding
import com.tradesk.databinding.ActivitySalesPersonDetailBinding
...
class SalesPersonDetailActivity : AppCompatActivity() {
    lateinit var binding: ActivitySalesPersonDetailBinding
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        if (intent.getStringExtra("phone").toString().isEmpty()) {
            ...
            binding.mTvPhone.text =
                Constants.insertString(intent.getStringExtra("phone").toString(), "", 0)
...
//            }
        ...
        if (intent.hasExtra("id")) {
            Constants.showLoading(this@SalesPersonDetailActivity)
            CoroutineScope(Dispatchers.IO).launch {
                viewModel.getSalesPersonDetail(intent.getStringExtra("id").toString(), "1", "10", intent.getStringExtra("type").toString(), ""
                )
            }
        }
    ...
    private fun setUserDetails(it: ClientSalesModelNew) {
        ...
        binding.mTvPhone.text = Constants.insertString(it.data.client.phone_no, "", 0)
        binding.mTvPhone.text = Constants.insertString(binding.mTvPhone.text.toString(), ")", 2)
        binding.mTvPhone.text = Constants.insertString(binding.mTvPhone.text.toString(), " ", 3)
        binding.mTvPhone.text = Constants.insertString(binding.mTvPhone.text.toString(), "-", 7)
        binding.mTvPhone.text = "+1 (" + binding.mTvPhone.text.toString()
